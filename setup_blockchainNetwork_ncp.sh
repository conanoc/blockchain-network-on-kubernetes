...
while [ "${JOBSTATUS}" != "1/1" ]; do
    ...
    sleep 2;
    if [[ "$(kubectl get pods | grep createchannel | awk '{print $3}')" == $'Error\nError' ]]; then
...
while [ "${JOBSTATUS}" != "1/1" ]; do
    ...
    sleep 2;
    if [[ "$(kubectl get pods | grep joinchannel | awk '{print $3}')" == $'Error\nError' ]]; then
        echo "Join Channel Failed"
        exit 1
    fi
    if [ "$(kubectl get pods | grep joinchannel | sed -e '$!d' | awk '{print $3}')" == "Completed" ]; then
        echo "One of the trials succeeded"
        break
    fi
    JOBSTATUS=$(kubectl get jobs |grep joinchannel |awk '{print $2}')
done
...
while [ "${JOBSTATUS}" != "1/1" ]; do
    ...
    sleep 2;
    if [[ "$(kubectl get pods | grep chaincodeinstall | awk '{print $3}')" == $'Error\nError' ]]; then
        echo "Chaincode Install Failed"
        exit 1
    fi
    if [ "$(kubectl get pods | grep chaincodeinstall | sed -e '$!d' | awk '{print $3}')" == "Completed" ]; then
        echo "One of the trials succeeded"
        break
    fi
    JOBSTATUS=$(kubectl get jobs |grep chaincodeinstall |awk '{print $2}')
...
while [ "${JOBSTATUS}" != "1/1" ]; do
    ...
    sleep 2;
    if [[ "$(kubectl get pods | grep chaincodeinstantiate | awk '{print $3}')" == $'Error\nError' ]]; then
